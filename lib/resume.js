...
exports.html = function(req, res) {
  function isodate(datestr) {
    var date = datestr.toLowerCase() == 'present' ?
      new Date() : new Date(datestr);
    return !isNaN(date.getTime()) && moment(date).format("YYYY-MM-DD");
  }

  // functions and variables used by Pug template
  res.locals.isodate = isodate;
  res.locals.today = moment().format('MMM D YYYY');
  res.locals.marked = marked;

  res.render('resume.pug', resumeData);
};
